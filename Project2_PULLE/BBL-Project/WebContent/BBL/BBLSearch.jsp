...
<%@page import="mainpage.service.SearchMainPageService"%>
...
<%
	...
	//핵심처리할 서비스 객체
	SearchMainPageService service = SearchMainPageService.getInstance();
	MainPageList dataList = null;
	//응답 데이터의 결과
	if(pageNumber==1){
		dataList = service.search(pageNumber, 9, word);
	}else{
		dataList = service.search(pageNumber, 3, word);
	}
	int num = dataList.getMainpageTotalCount();
%>
<script>
	var num = <%= num %>
</script>
