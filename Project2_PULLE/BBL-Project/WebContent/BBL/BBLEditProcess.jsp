...
<%@page import="mainpage.service.UpdateMainpageService"%>
...
<%
	try{
		...
		while(itr.hasNext()){
			...
			if(item.isFormField()){
				//타입="file"이 아닌경우
				if(item.getFieldName().equals("mainpageId")){
					mainpage.setMainpage_ID(Integer.parseInt(item.getString()));
				}
		...
		UpdateMainpageService update = UpdateMainpageService.getInstance();
		int cnt = 0;
		String msg ="";
		boolean chk= false;
		...
		try{
			cnt = update.update(mainpage, dir, userId);
			chk = true;
